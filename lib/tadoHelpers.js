            ...
            if (!getCallbacks[this.zone].processing) {
                ...
                tadoApi.getState(this.username, this.password, this.homeId, this.zone, (err, state) => {
                    if (err) {
                        ...
                        if (state.setting && state.setting.power !== 'OFF') {
                            const newOverlay = {
            ...
            if (!setCommands[this.zone].processing) {
                ...
                setTimeout(() => {
                    ...
                    if (commands['manualControl'] && commands['manualControl'] === 'OFF') {
                        if (this.debug) this.log('Got manualControl command: ', commands['manualControl'])
                        ...
                        if (commands['fanPower'] && commands['fanPower'] === 'ON') {
                            if (this.debug) this.log('Got fanPower command: ', commands['fanPower'])
                            overlayMode = 'FAN'
                        }
                        if (commands['fanSwing'] && this.capabilities['FAN'].swings) {
                            if (this.debug) this.log('Got fanSwing command: ', commands['fanSwing'])
                        ...
                        if (commands['fanFanspeed'] && this.capabilities['FAN'].fanSpeeds && this.capabilities['FAN'].fanSpeeds.includes(commands['fanFanspeed'])) {
                            if (this.debug) this.log('Got fanFanspeed command: ', commands['fanFanspeed'])
                        ...
                        if (commands['mode'] && commands['mode'] !== 'OFF') {
                            if (this.debug) this.log('Got mode command: ', commands['mode'])
                        ...
                        if (commands['temp']) {
                            if (this.debug) this.log('Got temp command: ', commands['temp'])
                            this.lastOverlay[overlayMode].setting.temperature.fahrenheit = Math.floor(commands['temp'] * 9 / 5 + 32)
                            this.lastOverlay[overlayMode].setting.temperature.celsius = commands['temp']
                        }

                        if (commands['coolTemp']) {
                            if (overlayMode === 'COOL') {
                                if (this.debug) this.log('Got coolTemp command: ', commands['coolTemp'])
                                this.lastOverlay[overlayMode].setting.temperature.fahrenheit = Math.floor(commands['coolTemp'] * 9 / 5 + 32)
                                this.lastOverlay[overlayMode].setting.temperature.celsius = commands['coolTemp']
                            }
                        }

                        if (commands['heatTemp']) {
                            if (overlayMode === 'HEAT') {
                                if (this.debug) this.log('Got heatTemp command: ', commands['heatTemp'])
                                this.lastOverlay[overlayMode].setting.temperature.fahrenheit = Math.floor(commands['heatTemp'] * 9 / 5 + 32)
                                this.lastOverlay[overlayMode].setting.temperature.celsius = commands['heatTemp']
                            }
                        }

                        if (commands['swing'] && this.capabilities[overlayMode].swings){
                            if (this.debug) this.log('Got swing command: ', commands['swing'])
                        ...
                        if (commands['fanspeed'] && this.capabilities[overlayMode].fanSpeeds && this.capabilities[overlayMode].fanSpeeds.includes(commands['fanspeed'])){
                            if (this.debug) this.log('Got fanspeed command: ', commands['fanspeed'])
