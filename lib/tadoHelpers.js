        ...
        updateAccessoryState: function(state, oldState) {
            ...
            if (this.forceThermostat || this.isThermostatic) {
                ...
                if (state.setting.power == 'OFF' || state.setting.mode == 'FAN' || state.setting.mode == 'DRY') {
                    if (oldState.setting.power !== 'OFF' && oldState.setting.mode !== 'FAN' && oldState.setting.mode !== 'DRY'){
                        if (this.debug) this.log('Updating Thermostat OFF for', this.name, '- Status -', state.setting.power == 'OFF' ? state.setting.power : state.setting.mode)
                    ...
                    if (this.debug) this.log('Updating Thermostat', this.name, '- Status -', state.setting.mode)
                    if (state.setting.mode == 'COOL') {
                        if (oldState.setting.mode !== state.setting.mode){
                        ...
                        if (!oldState.setting.temperature || oldState.setting.temperature.celsius !== state.setting.temperature.celsius){
                            if (this.debug) {
                                if (this.useFahrenheit)
                                    this.log('Updating Thermostat', this.name, '- Target Temperature -', state.setting.temperature.fahrenheit,'ºF')
                                else
                                    this.log('Updating Thermostat', this.name, '- Target Temperature -', state.setting.temperature.celsius,'ºC')
                            }
                ...
                if (oldState.sensorDataPoints.insideTemperature.celsius !== insideTemperature.celsius) {
                    if (this.debug) {
                        if (this.useFahrenheit)
                            this.log('Updating Thermostat', this.name, '- Current Temperature -', insideTemperature.fahrenheit,'ºF')
                        else
                            this.log('Updating Thermostat', this.name, '- Current Temperature -', insideTemperature.celsius,'ºC')
                    }
                    this.thermostatService.getCharacteristic(Characteristic.CurrentTemperature).updateValue(insideTemperature.celsius)
                }
                if (oldState.sensorDataPoints.humidity.percentage !== state.sensorDataPoints.humidity.percentage) {
                    if (this.debug) this.log('Updating Thermostat', this.name, '- CurrentRelativeHumidity -', state.sensorDataPoints.humidity.percentage)
                ...
                if (state.setting.power == 'OFF' || state.setting.mode == 'FAN' || state.setting.mode == 'DRY') {
                    if (oldState.setting.power !== 'OFF' && oldState.setting.mode !== 'FAN' && oldState.setting.mode !== 'DRY'){
                        if (this.debug) this.log('Updating HeaterCooler OFF for', this.name, '- Status -', state.setting.power == 'OFF' ? state.setting.power : state.setting.mode)
                    ...
                    if (oldState.setting.mode !== state.setting.mode){
                        ...
                        if (this.debug) this.log('Updating HeaterCooler ON for', this.name, '- Status -', state.setting.mode)
                    }

                    if (this.capabilities[state.setting.mode].swings && oldState.setting.swing !== state.setting.swing){
                        if (this.debug) this.log('Updating HeaterCooler', this.name, '- SwingMode -', state.setting.swing)
                    ...
                    if (this.capabilities[state.setting.mode].fanSpeeds && oldState.setting.fanSpeed !== state.setting.fanSpeed){
                        if (this.debug) this.log('Updating HeaterCooler', this.name, '- RotationSpeed -', state.setting.fanSpeed)
                    ...
                    if (state.setting.mode == 'COOL') {
                        ...
                        if (!oldState.setting.temperature || oldState.setting.temperature.celsius !== state.setting.temperature.celsius){
                            if (this.debug) {
                                if (this.useFahrenheit)
                                    this.log('Updating HeaterCooler', this.name, '- Cooling Threshold Temperature -', state.setting.temperature.fahrenheit,'ºF')
                                else
                                    this.log('Updating HeaterCooler', this.name, '- Cooling Threshold Temperature -', state.setting.temperature.celsius,'ºC')
                            }
                        ...
                        if (!oldState.setting.temperature || oldState.setting.temperature.celsius !== state.setting.temperature.celsius){
                            if (this.debug) {
                                if (this.useFahrenheit)
                                    this.log('Updating HeaterCooler', this.name, '- Heating Threshold Temperature -', state.setting.temperature.fahrenheit,'ºF')
                                else
                                    this.log('Updating HeaterCooler', this.name, '- Heating Threshold Temperature -', state.setting.temperature.celsius,'ºC')
                            }
                ...
                if (oldState.sensorDataPoints.insideTemperature.celsius !== insideTemperature.celsius) {
                    if (this.debug) {
                        if (this.useFahrenheit)
                            this.log('Updating HeaterCooler', this.name, '- Current Temperature -', insideTemperature.fahrenheit,'ºF')
                        else
                            this.log('Updating HeaterCooler', this.name, '- Current Temperature -', insideTemperature.celsius,'ºC')
                    }
                    this.HeaterCoolerService.getCharacteristic(Characteristic.CurrentTemperature).updateValue(insideTemperature.celsius)
                }
                
                if (!this.disableHumiditySensor && oldState.sensorDataPoints.humidity.percentage !== state.sensorDataPoints.humidity.percentage) {
                    if (this.debug) this.log('Updating HeaterCooler', this.name, '- CurrentRelativeHumidity -', state.sensorDataPoints.humidity.percentage)
            ...
            if (this.capabilities.FAN && !this.disableFan) {
                if (state.setting.power !== 'OFF' && state.setting.mode == 'FAN') {
                    if (oldState.setting.power === 'OFF' || oldState.setting.mode !== 'FAN') {
                        if (this.debug) this.log('Updating FAN Mode ON for', this.name)
                        this.FanService.getCharacteristic(Characteristic.Active).updateValue(1)
                    }
                    if (this.capabilities['FAN'].swings && oldState.setting.swing !== state.setting.swing){
                        if (this.debug) this.log('Updating FAN', this.name, '- SwingMode -', state.setting.swing)
                    ...
                    if (this.capabilities['FAN'].fanSpeeds && (oldState.setting.fanSpeeds !== state.setting.fanSpeeds || oldState.setting.mode !== 'FAN')){
                        if (this.debug) this.log('Updating FAN', this.name, '- RotationSpeed -', state.setting.fanSpeed)
                    ...
                    if (oldState.setting.mode == 'FAN') {
                        if (this.debug) this.log('Updating FAN Mode OFF for', this.name)
            ...
            if (this.manualControlSwitch) {
                if (!state.overlay && oldState.overlay){
                    if (this.debug) this.log('Updating Manual Control OFF for', this.name)
                    this.ManualSwitchService.getCharacteristic(Characteristic.On).updateValue(0)
                } else if (state.overlay && !oldState.overlay) {
                    if (this.debug) this.log('Updating Manual Control ON for', this.name)
